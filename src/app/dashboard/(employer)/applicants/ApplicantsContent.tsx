...
import CandidateCard from '@/components/dashboard/(employer)/applicants/CandidateCard';
...
export default function ApplicantsContent() {
    ...
    const [activeApplicant, setActiveApplicant] = useState<Candidate | null>(null);
    ...
    const changeStatus = async (applicantId: string, newStatus: 'applied' | 'shortlisted' | 'interviewed' | 'rejected' | 'hired') => {
        if (!jobId) return;
    ...
    return (
        <section className="p-6">
            ...
            {!jobId ? (
                <p className="text-gray-600 dark:text-gray-300">Invalid job ID.</p>
            ) : loading ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {applicants.map((candidate) => {
                        ...
                        const status = applied?.status || 'applied';
                        ...
                        return (
                            <CandidateCard
                                ...
                                onChangeStatus={(newStatus: string) => changeStatus(candidate._id, newStatus as 'applied' | 'shortlisted' | 'interviewed' | 'rejected' | 'hired')}
                            />
                        );
                    })}
