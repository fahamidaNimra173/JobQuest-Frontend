...
export default function ChangePasswordContent() {
  ...
  const breadcrumbItems = [
    { name: 'Change Password', href: '/dashboard/change-password', current: true }
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('success', 'Password changed successfully!', 'Your password has been updated.');
    } catch {
      showToast('error', 'Failed to change password', 'Please try again or contact support.');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start">
          ...
          <div>
            <h3 className="text-sm font-medium text-primary-dark mb-1">Security Tips</h3>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full transition-all duration-300 ${
                      passwordStrength.strength === 'strong' ? 'bg-primary-dark' : passwordStrength.color
                    }`}
                ...
                {passwordRequirements.map((requirement) => {
                  ...
                  return (
                    <div key={requirement.id} className="flex items-center">
                      ...
                      <span className={`text-xs ${isValid ? 'text-primary-dark' : 'text-gray-600'}`}>
                        {requirement.text}
                      </span>
                    </div>
                  );
